...
function viewReader(stationId){
  ...
  const nav='<div class="reader-nav">'
    + (prev?'<button onclick="go(\'#/r/umrah/'+prev.id+'\')">'+ICONS.back
        +'<span class="lbl"><small>پێشوو</small>'+esc(prev.title.slice(0,18))+'</span></button>'
        :'<button disabled>'+ICONS.back+'<span class="lbl"><small>پێشوو</small>—</span></button>')
    + (next?'<button onclick="go(\'#/r/umrah/'+next.id+'\')"><span class="lbl"><small>دواتر</small>'+esc(next.title.slice(0,18))+'</span>'+ICONS.chev+'</button>'
        :'<button disabled><span class="lbl"><small>دواتر</small>—</span>'+ICONS.chev+'</button>')
    +'</div>';
